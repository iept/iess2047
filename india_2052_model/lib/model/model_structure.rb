...
class ModelStructure < ModelUtilities
  ...
  def types
    @types ||= (5..53).to_a.map { |row| excel.send("control_f#{row}") }
  end
  
  def names
    @names ||= (5..53).to_a.map { |row| excel.send("control_d#{row}") }
  end

  def descriptions
    @descriptions ||= (5..53).to_a.map { |row| [r("control_h#{row}"),r("control_i#{row}"),r("control_j#{row}"),r("control_k#{row}")] }
  end

  def long_descriptions
    @long_descriptions ||=  (5..53).to_a.map  { |row| [r("control_bo#{row}"), r("control_bp#{row}"),r("control_bq#{row}"),r("control_br#{row}")] }
  end
    ...
    ('m'..'z').to_a.push('aa','ab').map do |column|
      {
        ...
        code: convert_float_to_letters((5..53).map { |row| r("control_#{column}#{row}") }).join,
